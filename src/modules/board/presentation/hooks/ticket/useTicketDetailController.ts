...
import { useComments } from "@/modules/board/presentation/hooks/comment";
import {
  useAssignTicket,
  useDeleteTicket,
  useTicket,
...
export const useTicketDetailController = ({
  ...
  const canSave =
    hasDirtyFields && !!ticket && canEditTicket && !!effectiveTitle.trim();
  const { autoSaveState } = useAutoSave(canSave, handleSaveMainFields);
  ...
  const {
    ...
    handleSaveComment,
    handleDeleteComment,
  } = useCommentActions(ticketId, canComment);
